...
// Enhanced register endpoint that properly handles restaurant creation
router.post('/register', async (req, res) => {
  try {
    const { 
      name, email, phone, password, role = 'customer', 
      license, vehicle, owner, license_number, cuisine_type,
      restaurant_location, delivery_fee, description, image_url, status
    } = req.body;

    console.log('Register request:', { name, email, phone, role, hasPassword: !!password });
    ...
    if (!email || !phone || !password) {
      return res.status(400).json({
        ...
        errors: ['البريد الإلكتروني والهاتف وكلمة المرور مطلوبة']
      });
    }
    ...
    if (role === 'restaurant') {
      if (!owner || !license_number || !cuisine_type || !restaurant_location) {
        return res.status(400).json({
          success: false,
          message: 'بيانات ناقصة للمطعم',
          errors: ['اسم المالك ورقم الترخيص ونوع المأكولات والموقع مطلوبة']
        });
      }
      
      // Use restaurant controller for restaurant creation
      req.body.name = name || owner; // Use owner as name if name not provided
      return require('../controllers/restaurantController').createRestaurant(req, res);
    }
    ...
    // Prepare user data
    const userData = {
      name: name ? name.trim() : (owner ? owner.trim() : email.split('@')[0]),
      email: email.toLowerCase().trim(),
      phone: phone.trim(),
      password,
      role: role || 'customer',
    ...
    switch (role) {
      case 'driver':
        userData.license = license.trim();
        userData.vehicle = vehicle.trim();
      ...
      case 'delivery':
        ...
        userData.rating = 5.0;
        userData.location = { lat: 30.0444, lng: 31.2357 };
        userData.isLocationActive = false;
        userData.locationAccuracy = null;
